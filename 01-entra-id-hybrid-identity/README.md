...
![Starting point — single unlicensed user, no groups](./01_bulk_users/01_before_single_user.png.png)

![Initial bulk create failure](./01_bulk_users/02_bulk_create_error.png.png)

![Bulk create error detail](./01_bulk_users/03_bulk_create_error.png.png)

![Corrected submission succeeding](./01_bulk_users/04_bulk_create_success.png.png)

![33 users confirmed](./01_bulk_users/05_bulk_create_confirmation.png.png)
...
![Four groups created](./02_dynamic_groups/01_four_groups_created.png.png)

![Dynamic rule error caught](./02_dynamic_groups/02_dynamic_rule_error_caught.png.png)

![Dynamic rule corrected](./02_dynamic_groups/03_dynamic_rule_corrected.png.png)

![Groups — Dynamic vs Assigned membership types confirmed](./02_dynamic_groups/04_groups_dynamic_vs_assigned.png)
...
![Assigning license at group level in M365 admin center](./03_licensing/01_m365_assigning_license.png.png)

![License assignment confirmation](./03_licensing/02_license_assignment_confirmation.png)

![4 groups, 4 membership types](./03_licensing/03_4_groups_4_types.png)
...
**Deliverable:** [`Lab01_RBAC_Matrix.xlsx`](./Lab01_RBAC_Matrix.xlsx) — includes a Legend tab explaining each column, used as the working reference for Steps 8–9 below.
...
![Graph/PowerShell permission consent granted](./04_rbac_custom_roles/01_graph_powershell_consent_granted.png)

![Custom role created](./04_rbac_custom_roles/02_custom_role_created.png)

![Permission replaced, not additively stacked](./04_rbac_custom_roles/03_permission_replaced_not_added.png)
...
![IT Manager Role activation settings](./05_pim/01_it_manager_role_activation_settings.png)

![IT Manager Role settings saved](./05_pim/02_it_manager_role_settings_saved.png)

![Global Administrator activation settings](./05_pim/03_global_admin_activation_settings_.png)

![Eligible assignment created](./05_pim/04_eligible_assignment_created.png)

![Activation success — Stage 3 confirmation](./05_pim/05_activation_success.png)

![Pre-activation "My roles" eligible assignments view](./05_pim/06_before_activate_button.png)

![PIM audit trail — settings, eligibility, and activation events](./07_audit_logs/01_pim_activation_audit_entry.png)
...
![Non-gallery app creation dialog](./06_sso_saml/01_create_custom_app_dialog.png)

![SAML Basic Configuration — Entity ID and Reply URL saved](./06_sso_saml/02_saml_basic_configuration.png)
...
![SAML decoded claims — Issuer, Signature status, NameID, Conditions, Attributes](./06_sso_saml/03_saml_decoded_sample_claims.png)

![Raw SAML XML structure](./06_sso_saml/03-1_saml_html_sample.png)

![JWT header](./06_sso_saml/04_jwt_json_header.png)

![JWT payload](./06_sso_saml/04-1_jwt_json_payload.png)

![JWT header + payload + signature verification with claims breakdown](./06_sso_saml/04-2_jwt_json_header_payload_verification_claims_breakdown.png)
...
**Evidence:** See PIM audit trail screenshot in Step 9–10 above (`07_audit_logs/01_pim_activation_audit_entry.png`).
